...
(function() {
	...
	var FunctionH = {
		 ...
		 * @return {int}  返回setTimeout的timerId
		 */
		 ...
		 * 创建并返回一个像节流阀一样的函数，当重复调用函数的时候，
		 最多每隔 wait毫秒调用一次该函数。对于想控制一些触发频率较高的事件有帮助。
		 * @method throttle
		 * @static
		 * @param {Function} fun  调用函数
		 * @param {Number} wait  延迟毫秒数
		 * @param {Object} options 选项 默认情况下，throttle将在你调用的第一时间尽快执行这个function，
		 并且，如果你在wait周期内调用任意次数的函数，都将尽快的被覆盖。
		 如果你想禁用第一次首先执行的话，传递{leading: false}，
		 还有如果你想禁用最后一次执行的话，传递{trailing: false}。
		 * @return {Function}  返回处理过的function
		*/
		 ...
		 * @param {Function} fun  调用函数
		 * @param {Number} wait  延迟毫秒数
		 * @param {Booler} immediate 传参 immediate 为 true 会让debounce 在 wait 间隔之后
		  触发最后的函数调用而不是最先的函数调用.
